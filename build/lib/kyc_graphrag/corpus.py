...
def load_documents(policy_dir: Path) -> list[Document]:
    ...
    for path in sorted(policy_dir.rglob("*")):
        ...
        if doc_id in used_ids:
            relative = path.relative_to(policy_dir).as_posix().replace("/", "-")
            digest = hashlib.sha1(relative.encode()).hexdigest()[:6]
            doc_id = f"{path.stem}-{digest}"
        used_ids.add(doc_id)
        docs.append(Document(doc_id=doc_id, title=title, text=text, path=str(path)))
    return docs
    ...
    for doc in docs:
        ...
        for para in paragraphs:
            if buf and len(buf) + len(para) > max_chars:
                chunks.append(
                    {"chunk_id": f"{doc.doc_id}:{part}", "doc_id": doc.doc_id, "text": buf}
                )
                part += 1
                buf = para
            else:
