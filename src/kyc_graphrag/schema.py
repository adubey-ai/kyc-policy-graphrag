...
ENTITIES: tuple[MentionSpec, ...] = (
    ...
    MentionSpec("Dual Checker", "Role", ("dual checker", "two checkers", "dual sign-off", "dual checker sign-off")),
    MentionSpec("IASW Agent", "System", ("iasw", "iasw agent", "iasw pipeline", "iasw packet", "field-extraction bot", "extraction microservice", "extraction bot", "extraction service", "packet builder")),
    MentionSpec("CBS", "System", ("cbs", "core banking", "core-banking", "core banking system", "customer-master", "customer master")),
    ...
    MentionSpec("PEP", "CustomerType", ("pep", "politically exposed person", "politically exposed")),
    ...
    MentionSpec("Address Change", "Process", ("address change", "address update", "change of address")),
...
RELATION_CUES: tuple[tuple[tuple[str, ...], frozenset[str], frozenset[str], str], ...] = (
    (
        ("cannot post", "cannot write", "must not post", "barred from", "prohibited from", "forbidden from", "no write", "no cbs write", "inherit maker restriction"),
        frozenset({"Role", "System", "Unit"}),
        frozenset({"System"}),
        "CANNOT_WRITE",
    ),
    (
        ("only the checker", "checker is the only", "write access", "can approve", "trigger the cbs", "may post", "can write"),
        frozenset({"Role"}),
        frozenset({"System"}),
        "CAN_WRITE",
    ),
    ...
    (
        ("requires", "require", "mandatory", "must submit", "need a", "needs a", "additionally requires", "triggers edd", "trigger edd"),
